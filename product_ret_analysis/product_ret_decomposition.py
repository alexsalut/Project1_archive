...
class ProductRetDecomposition:
    ...
    def gen_email(self):
        table_1, table_2 = self.gen_table()
        styled_1, styled_2 = self.format_table(table_1), self.format_table(table_2)
        content = f"""
    ...
    def gen_df(self):
        ...
        for product in self.stock_list:
            s = pd.Series(self.get_ret_decomposition(product), name=product)
            data.append(s)
    ...
    def get_ret_decomposition(self, product):
        product_dict = {}
        stock_asset = get_product_record(product, '股票资产总值', self.date)

        product_dict['股票收益率1'] = get_product_record(product, '股票盈亏', self.date) / stock_asset
        product_dict['股票交易收益率'] = self.get_trade_pl(product, 'Credit') / stock_asset
        ...
        if product in self.option_list:
            ...
            product_dict['期权交易收益率'] = self.get_trade_pl(product, 'Option') / stock_asset
            ...
            product_dict['期权权重配置误差'] = product_dict['期权持有收益率'] + product_dict['指数收益率'] + product_dict['ETF跟踪误差'] - product_dict['期权基差']
    ...
    def get_trade_pl(self, product, type):
        ...
        if ticker_list:
            new_trade_df = pd.concat([trade_df, get_t_raw_daily_bar(ticker_list=ticker_list, type=type, date=self.date)], axis=1)
            trade_pl = new_trade_df.apply(
                lambda x: x['成交数量'] * x['close'] - x['成交金额'] if x['成交金额'] > 0
                else -x['成交数量'] * x['close'] - x['成交金额'], axis=1).sum()
        else:
            trade_pl = 0
...
def get_t_raw_daily_bar(ticker_list, type, col='close', date=None):
    if type == 'Option':
        rq.init()
        raw_daily = rq.get_price(ticker_list, start_date=date, end_date=date).droplevel(1)
...
if __name__ == '__main__':
    ProductRetDecomposition('20231205').gen_email()
