...
def get_filtered_convertible_pair(premium_thresh=0.05, date=None):
    ...
    st_stock_lst = get_last_5days_st_stock()
    ...
    data = rq.convertible.instruments(pair.index.tolist())
    data_lst = [dict(value) for value in data]
    conv_df = pd.DataFrame(data_lst)
    conversion_convs = conv_df.query('conversion_start_date <= @formatted_date < conversion_end_date')['order_book_id'].tolist()
    ...
    filtered_pair_df = pair_df.query(f'(last_premium <= {premium_thresh} and rq_type!="None") or stock_code in {rq_volume.index.tolist()}')
