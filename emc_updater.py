...
def check_file(actual_file_path, target_file_path):
    ...
    if os.path.exists(actual_file_path) & os.path.exists(target_file_path):
        print(f'{file_name} exists.')
        actual_df = pd.read_csv(actual_file_path, encoding='gbk').rename(columns={'持仓数量':'实际持仓数量'})
        filter_df = actual_df.query('实际持仓数量 != 0').sort_values(by='市值', ascending=False).reset_index(drop=True)
