...
from record.account_terminal_reader import read_terminal_file
...
def get_tinglian2_info(date=None):
    emc_tinglian_dir = FileLocation.account_dir['听涟2号 emc']
    emc_account_code = FileLocation.account_code["听涟2号"]
    cats_tinglian_dir = FileLocation.account_dir['听涟2号']
    ...
    cats_option_info = read_terminal_file(cats_tinglian_dir, '中信期权账户', cats_option_code, date)

    info_dict = {
        '期权权益': cats_option_info['账户净资产'],
        '股票权益': emc_info['账户净资产'],
        '股票市值': emc_info['证券市值'],
        '成交额': emc_info['成交额'] + cats_option_info['成交额'],
        '股票成交额': emc_info['成交额'],
        '期权成交额': cats_option_info['成交额'],
        '股票交易明细': emc_info['交易明细'],
        '期权交易明细': cats_option_info['交易明细']
    }
    return info_dict
...
def get_talang23_info(account, date=None):
    ...
    account_dir = FileLocation.account_dir[account]
    account_info = read_terminal_file(account_dir, '华安普通账户', account_code, date)
    return account_info


def get_talang1_info(date=None):
    account_dir = FileLocation.account_dir['踏浪1号']
    stock_account_code = FileLocation.account_code['踏浪1号']
    option_account_code = FileLocation.option_account_code['踏浪1号']
    stock_info = read_terminal_file(account_dir, '中信信用账户', stock_account_code, date)
    option_info = read_terminal_file(account_dir, '中信期权账户', option_account_code, date)
    option_transaction_vol = option_info['成交额']
    stock_transaction_vol = stock_info['成交额']
    ...
    info_dict = {
        ...
        '成交额': stock_transaction_vol + option_info['成交额'],
        '股票成交额': stock_transaction_vol,
        '期权成交额': option_transaction_vol,
        '股票交易明细': stock_info['交易明细'],
        '期权交易明细': option_info['交易明细']
...
def get_panlan1_info(date=None):
    account_dir = FileLocation.account_dir['盼澜1号']
...
def get_nongchao1_info(date=None):
    account_dict = CatsFileReader(
        file_dir=FileLocation.account_dir['弄潮1号 cats'],
        account_code=FileLocation.account_code['弄潮1号'],
        date=date
    ).get_cats_account_info(account_lst=['中信普通账户', '中信信用账户', '中信期权账户'])

    matic_account_dict = MaticFileReader(
        file_dir=FileLocation.account_dir['弄潮1号 matic'],
        account_code='衍舟弄潮1号',
        date=date
...
def get_nongchao2_info(date=None):
    account_dict = MaticFileReader(
        file_dir=FileLocation.account_dir['弄潮2号 matic'],
        account_code='衍舟弄潮2号',
        date=date
    ).get_matic_account_info()
    return account_dict
